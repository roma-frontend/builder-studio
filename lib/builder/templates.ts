...
export const TEMPLATES: TemplateDef[] = [
  ...
  },
  {
    ...
    description: 'Готовая страница политики конфиденциальности.',
    suggestedPath: 'privacy',
    build: () => ({
      id: newId('page'),
      path: 'privacy',
      ...
      blocks: [
        pageHeader('Политика конфиденциальности', 'Мы уважаем вашу приватность и заботимся о защите ваших персональных данных.'),
        legalSection([
          [LEGAL_NOTE, true],
          ['Какие данные мы собираем', false],
          ['Мы собираем данные, которые вы предоставляете сами (имя, адрес электронной почты, телефон и содержание обращений), а также данные, которые собираются автоматически при использовании сайта: IP-адрес, тип браузера и устройства, страницы просмотра и файлы cookie.', true],
          ['Как мы используем данные', false],
          ['Мы используем ваши данные, чтобы предоставлять и улучшать наши услуги, отвечать на ваши запросы, обеспечивать безопасность сайта, отправлять важные уведомления и, с вашего согласия, информационные и маркетинговые сообщения.', true],
          ['Файлы cookie', false],
          ['Мы используем файлы cookie и аналогичные технологии, чтобы запоминать ваши настройки, анализировать посещаемость и повышать удобство сайта. Вы можете отключить cookie в настройках браузера, однако часть функций может стать недоступной.', true],
          ['Передача данных третьим лицам', false],
          ['Мы не продаём ваши персональные данные. Мы можем передавать их поставщикам услуг (хостинг, аналитика, платежи), которые действуют по нашему поручению, а также государственным органам, когда этого требует закон.', true],
          ['Хранение данных', false],
          ['Мы храним персональные данные не дольше, чем это необходимо для целей, ради которых они были собраны, либо в течение срока, установленного законом. После этого данные удаляются или обезличиваются.', true],
          ['Ваши права', false],
          ['Вы вправе запросить доступ к своим данным, их исправление или удаление, ограничить или возразить против обработки, а также отозвать ранее данное согласие. Для реализации этих прав свяжитесь с нами.', true],
          ['Контакты', false],
          ['По всем вопросам, связанным с обработкой персональных данных, вы можете написать нам на электронную почту, указанную на сайте. Мы ответим в разумный срок.', true],
        ]),
      ],
    }),
  },
  {
    ...
    description: 'Готовая страница пользовательского соглашения.',
    suggestedPath: 'terms',
    build: () => ({
      id: newId('page'),
      path: 'terms',
      ...
      blocks: [
        pageHeader('Условия использования', 'Пожалуйста, внимательно ознакомьтесь с условиями перед использованием сайта.'),
        legalSection([
          [LEGAL_NOTE, true],
          ['Принятие условий', false],
          ['Используя наш сайт и услуги, вы соглашаетесь с настоящими условиями использования. Если вы не согласны с ними, пожалуйста, прекратите использование сайта.', true],
          ['Учётные записи', false],
          ['Для доступа к некоторым функциям может потребоваться регистрация. Вы обязаны предоставлять достоверные данные, сохранять конфиденциальность пароля и несёте ответственность за все действия в вашей учётной записи.', true],
          ['Пользовательский контент', false],
          ['Вы сохраняете права на материалы, которые размещаете, но предоставляете нам право использовать их для работы сервиса. Вы обязуетесь не публиковать незаконный, оскорбительный или нарушающий чужие права контент.', true],
          ['Интеллектуальная собственность', false],
          ['Все материалы сайта — тексты, графика, логотипы и программный код — являются нашей собственностью или собственностью наших партнёров и защищены законом. Их использование без разрешения запрещено.', true],
          ['Отказ от гарантий', false],
          ['Сайт и услуги предоставляются «как есть» без каких-либо гарантий. Мы не гарантируем бесперебойную работу, отсутствие ошибок или соответствие сайта вашим конкретным ожиданиям.', true],
          ['Ограничение ответственности', false],
          ['В максимально допустимой законом степени мы не несём ответственности за косвенные убытки, упущенную выгоду или потерю данных, возникшие в результате использования или невозможности использования сайта.', true],
          ['Прекращение доступа', false],
          ['Мы вправе приостановить или прекратить ваш доступ к сайту в случае нарушения настоящих условий, без предварительного уведомления и без обязательств по возмещению.', true],
          ['Контакты', false],
          ['Если у вас есть вопросы по условиям использования, свяжитесь с нами по электронной почте, указанной на сайте.', true],
        ]),
      ],
    }),
  },
  {
    ...
    description: 'Готовая страница политики использования cookie.',
    suggestedPath: 'cookies',
    build: () => ({
      id: newId('page'),
      path: 'cookies',
      title: 'Политика cookie',
      description: 'Как и зачем мы используем файлы cookie на нашем сайте.',
      blocks: [
        pageHeader('Политика cookie', 'Этот документ объясняет, как мы используем файлы cookie и как вы можете ими управлять.'),
        legalSection([
          [LEGAL_NOTE, true],
          ['Что такое файлы cookie', false],
          ['Файлы cookie — это небольшие текстовые файлы, которые сайт сохраняет на вашем устройстве. Они помогают запоминать ваши действия и настройки, чтобы вам не приходилось вводить их повторно при каждом посещении.', true],
          ['Какие cookie мы используем', false],
          ['Мы используем необходимые cookie для работы сайта, функциональные cookie для запоминания ваших настроек, аналитические cookie для оценки посещаемости и маркетинговые cookie для показа релевантной рекламы.', true],
          ['Управление cookie', false],
          ['Вы можете управлять файлами cookie и удалять их через настройки браузера, а также с помощью баннера согласия на сайте. Обратите внимание, что отключение отдельных cookie может ограничить работу сайта.', true],
          ['Изменения политики', false],
          ['Мы можем время от времени обновлять эту политику cookie. Актуальная версия всегда доступна на этой странице, а дата последнего изменения указывается в её начале.', true],
          ['Контакты', false],
          ['Если у вас есть вопросы об использовании файлов cookie, напишите нам на электронную почту, указанную на сайте.', true],
        ]),
      ],
    }),
  },
...
// Shared editable-template disclaimer shown at the top of every legal page.
const LEGAL_NOTE = 'Это готовый шаблон — замените текст на условия вашей компании и проконсультируйтесь с юристом. «Наш сайт» и «наша компания» — примеры, которые нужно заменить своим названием.';

// Build a legal page body from [text, muted] pairs: muted:false → an h2
// heading, muted:true → a muted paragraph. Same left-aligned layout for all
// legal templates, fully editable in the builder.
const legalSection = (items: [string, boolean][]): BuilderNode =>
  mk('section', { padding: 'lg', bg: 'none', width: 'normal' }, items.map(([text, muted]) =>
    muted
      ? mk('text', { text, align: 'left', muted: 'true' })
      : mk('heading', { text, level: '2', align: 'left' }),
  ));
