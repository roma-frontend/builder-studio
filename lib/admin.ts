...
export function assignSiteAdmin(siteId: string, email: string): { id: string; email: string; name: string } {
  ...
  if (!target) {
    ...
    const created: User = {
      ...
      role: 'admin', isActive: true, failedAttempts: 0, lockedUntil: null, totpSecret: null, totpEnabled: false, mustChangePassword: false, telegramId: null, telegramUsername: null, googleId: null, appleId: null, createdAt: now,
    };
