...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (code === 'SLUG_TAKEN') return NextResponse.json({ error: 'Адрес организации уже занят.' }, { status: 409 });
    if (code === 'ALREADY_REVIEWED') return NextResponse.json({ error: 'Заявка уже рассмотрена.' }, { status: 409 });
    if (code === 'REQUEST_NOT_FOUND') return NextResponse.json({ error: 'Заявка не найдена.' }, { status: 404 });
