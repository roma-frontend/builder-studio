  ...
  return (
    <div className="space-y-4">
      <div className="relative max-w-md">
        ...
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Поиск по имени, email, организации" className="h-10 pl-10" />
      </div>

      {filtered.length === 0 ? (
        <div className="flex h-40 items-center justify-center rounded-2xl border border-border/60 text-sm text-muted-foreground">
          <Users className="mr-2 h-5 w-5" /> Пользователей нет.
        </div>
      ) : (
            ...
            return (
              <div key={u.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-border/60 bg-card px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">{u.name || 'Без имени'} <span className={`ml-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ${meta.cls}`}>{meta.label}</span></p>
                  <p className="truncate text-xs text-muted-foreground">{u.email} · сейчас: {u.siteName} (/s/{u.siteSlug})</p>
                </div>
                ...
                <Button size="sm" className="gap-1.5" disabled={busy === u.id || target === u.siteId} onClick={() => assign(u)}>
                  {busy === u.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} Присвоить
                </Button>
                {u.status !== 'approved' ? (
                  <Button size="sm" variant="outline" disabled={busy === u.id} onClick={() => setStatus(u, 'approved')}>Одобрить</Button>
                ) : (
                  <Button size="sm" variant="outline" className="border-red-500/40 text-red-500 hover:bg-red-500/10" disabled={busy === u.id} onClick={() => setStatus(u, 'suspended')}>Приостановить</Button>
                )}
              </div>
            );
          })}
